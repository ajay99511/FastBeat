...
class MediaDaoTest {
    ...
    private fun history(
        ...
        mediaId, position, duration, timestamp, mediaType, audioTrackIndex, subtitleTrackIndex
    )

    // ---------------------------------------------------------------- playback_history

    @Test
    fun saveHistory_roundTripsEveryField() = runBlocking {
        dao.saveHistory(
            history(
                mediaId = 1, position = 4_200, duration = 60_000, timestamp = 999,
                mediaType = "AUDIO", audioTrackIndex = 2, subtitleTrackIndex = 3
            )
        )
    ...
    @Test
    fun saveHistory_replacesRowWithSameMediaId() = runBlocking {
    ...
    @Test
    fun getHistory_returnsNullForUnknownMediaId() = runBlocking {
     ...
     */
    @Test
    fun updateHistoryPosition_leavesMediaTypeAndTrackSelectionsIntact() = runBlocking {
        dao.saveHistory(
            history(
                mediaId = 1, position = 10, duration = 20, timestamp = 1,
                mediaType = "VIDEO", audioTrackIndex = 5, subtitleTrackIndex = 7
            )
        )
    ...
    @Test
    fun getLastPlayed_returnsTheMostRecentRowByTimestamp() = runBlocking {
    ...
    @Test
    fun getLastPlayedAudio_ignoresVideoRows() = runBlocking {
    ...
    @Test
    fun getContinueWatching_excludesAudio() = runBlocking {
    ...
    @Test
    fun getContinueWatching_excludesUnstartedVideo() = runBlocking {
    ...
    @Test
    fun getContinueWatching_excludesVideoWatchedPast95Percent() = runBlocking {
    ...
    @Test
    fun getContinueWatching_includesPartiallyWatchedVideo() = runBlocking {
    ...
    @Test
    fun getContinueWatching_includesVideoWithUnknownDuration() = runBlocking {
    ...
    @Test
    fun getContinueWatching_ordersMostRecentFirst() = runBlocking {
    ...
    @Test
    fun initAnalytics_doesNotOverwriteAnExistingRow() = runBlocking {
    ...
    @Test
    fun incrementPlayCount_accumulatesAndRecordsLastPlayed() = runBlocking {
        dao.initAnalytics(mediaId = 1, timestamp = 10)
        dao.incrementPlayCount(mediaId = 1, timestamp = 20)
    ...
    @Test
    fun incrementSkipCount_doesNotTouchPlayCount() = runBlocking {
        dao.initAnalytics(mediaId = 1, timestamp = 10)
        dao.incrementPlayCount(mediaId = 1, timestamp = 20)
    ...
    @Test
    fun replaceQueue_swapsTheWholeQueueRatherThanMerging() = runBlocking {
    ...
    @Test
    fun replaceQueue_preservesSortOrder() = runBlocking {
        dao.replaceQueue(
    ...
    @Test
    fun replacePlaylistMedia_onlyAffectsTheTargetPlaylist() = runBlocking {
    ...
    @Test
    fun getMediaIdsForPlaylist_ordersByAddedAt() = runBlocking {
        dao.insertPlaylist(PlaylistEntity("a", "A", createdAt = 1, isVideo = false))
    ...
    @Test
    fun deletingPlaylist_cascadesToItsCrossRefs() = runBlocking {
        dao.insertPlaylist(PlaylistEntity("a", "A", createdAt = 1, isVideo = false))
    ...
    @Test
    fun addToDailyPlaytime_accumulatesWithinTheSameDay() = runBlocking {
    ...
    @Test
    fun getActiveDays_dropsDaysUnderThresholdAndOrdersNewestFirst() = runBlocking {
     ...
     */
    @Test
    fun deletionQueries_removeTheMediaFromEveryTableThatReferencesIt() = runBlocking {
        val gone = 1L
        val kept = 2L
