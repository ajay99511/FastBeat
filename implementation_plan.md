...
| P4-E.2 | Extract `PlaybackAnalyticsTracker` | 🟨 | P4-E.1 | ✅ *(pending device test)* | `playback/PlaybackAnalyticsTracker.kt` [NEW] — owns `recordPlay`, `recordSkip`, the per-track accumulator, `hasLoggedCurrentTrack` and daily-playtime flushing. The 500 ms loop in `PlaybackViewModel` drops from ~45 lines of analytics to **6**. **Key invariant held:** `onTrackChanged()` resets the per-track accumulator on every transition — without it a long listen would instantly satisfy the next track's threshold and record a play the user never made. **Faithful port, three details preserved deliberately:** (1) flush cadence checks `tick % 60` *before* incrementing, so the first tick of a session still flushes immediately, exactly as the original did; (2) `mediaId` is **nullable** — the original accrued daily playtime whenever playback was playing even if the track was unresolved, so restricting accrual to known tracks would have quietly changed the numbers; (3) writes stay fire-and-forget on an IO scope so the position loop is never suspended. Two pre-existing bugs found and **deliberately not fixed here** (F-29, F-30) — changing what analytics report inside a refactor commit would make the refactor unreviewable. Gate: `assembleDebug lint testDebugUnitTest detekt ktlintCheck` → BUILD SUCCESSFUL, **84 tests / 0 failures**; neither baseline mentions the new file. ⏳ Device test pending. |
...
| F-33 | P4-F.2 | Externalisation is **not finished app-wide**. `LibraryViewModel` still emits ~12 hardcoded English strings (rename validation, delete failures) through its own `SharedFlow<String>`, and `EqualizerSheet` still hardcodes its labels (F-6). P4-F.2's card scoped only `PlaybackViewModel`, so these are out of scope rather than missed — but the app is not translatable until they follow. Worth one more pass. | 🟡 Major |
...
| F-29 | P4-E.2 | **Up to 30 s of listening time is discarded every time playback stops.** Daily playtime accumulates in memory and flushes only every 60 ticks; `stopPositionUpdates()` cancels the job without flushing, so whatever accrued since the last flush is lost. Present in the original code and preserved verbatim by the extraction. Now a one-line fix (`analytics.flushPending()` in `stopPositionUpdates`) but it changes reported totals, so it belongs in its own commit — not folded into a refactor. | 🟡 Major |
| F-30 | P4-E.2 | Playtime is credited to the day the **session started**, because `getNormalizedToday()` was captured once outside the loop. A session running past midnight books all of its time to the previous day. Pre-existing; preserved by the extraction. Fix alongside F-29. | 🟢 Minor |
...
| **OQ-7** | Should a blanket `.fallbackToDestructiveMigration()` backstop be added (P3-C card Step 3)? It prevents a crash loop on an unforeseen version, but makes every *future* forgotten migration a silent data wipe. | Future schema bumps | Owner | **Not added.** `fallbackToDestructiveMigrationFrom(2,3,4)` + `MIGRATION_1_5` covers every shipped version; failing loudly on an unknown one is preferable while the app is under active schema work |
