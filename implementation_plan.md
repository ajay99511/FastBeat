...
| F-34 | P4-E.2 | **Up to 30 s of listening time is discarded every time playback stops.** Daily playtime accumulates in memory and flushes only every 60 ticks; `stopPositionUpdates()` cancels the job without flushing, so whatever accrued since the last flush is lost. Present in the original code and preserved verbatim by the extraction. Now a one-line fix (`analytics.flushPending()` in `stopPositionUpdates`) but it changes reported totals, so it belongs in its own commit — not folded into a refactor. | 🟡 Major |
| F-30 | P4-E.2 | Playtime is credited to the day the **session started**, because `getNormalizedToday()` was captured once outside the loop. A session running past midnight books all of its time to the previous day. Pre-existing; preserved by the extraction. Fix alongside F-34. | 🟢 Minor |
