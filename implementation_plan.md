...
| P4-E.4 | Extract `QueueManager` | 🟥 | P4-E.1 | ✅ **All three OQ-9 steps landed** | Step 1 = `QueuePolicy` + 22 characterization tests. **Step 2 = `playback/QueuePersistence.kt` [NEW]** — every write to and read from the saved audio session now lives behind one class: the Room `current_queue` rows, the four scalar prefs (`last_queue_index`, `last_shuffle_enabled`, `last_repeat_mode`, `last_playlist_context`) and the `saved_audio_session` JSON snapshot. This is what the invariant needed: previously the persisted queue could be written from ~35 sites scattered through a 2 250-line file, so *what can change the queue on disk?* had no answer. The 8 private `persist*`/`*SavedAudioState` helpers were kept as one-line delegations so **none of the ~35 call sites changed** — extract-and-delegate, not rewrite. `sharedPrefs` references in the ViewModel: 12 → **3** (all `video_brightness`, unrelated). ViewModel 2 257 → **2 178** lines. **A real defect was found by the new tests** — see F-31. Full suite **123 tests / 0 failures**; neither baseline mentions the new files. | Measured before starting: the four queue StateFlows (`_currentQueue`, `_displayQueue`, `_currentIndex`, `_currentPlaylistContext`) have **81 references** spread across the whole ViewModel — `setQueue`, every play-from-{playlist,album,artist,smart,decade} entry point, restore-on-launch, `completeCurrentTrackDelete`, the `Player.Listener`, shuffle, PiP. Unlike E.1/E.2/E.3 there is no self-contained mechanism to lift; the queue state **is** the ViewModel's spine. Combined with: this task's own invariant (*Room and ExoPlayer must never disagree on queue content or order*), §Residual risk naming it “the most likely place to introduce a subtle regression”, **zero automated coverage of queue behaviour** (P4-G.2 comes *after* this task), and two extractions still awaiting device verification — proceeding blind was judged reckless. **E.5 was landed first** (independent, 🟩, depends only on E.1) while the approach is decided. See the three options recorded in §Open questions as OQ-9. **Step 3 (final) = `playback/QueueManager.kt` [NEW]** — the four StateFlows and their 81 references. The extraction was mechanical (every reference is a `.value` read or write), so it was done as one scripted pass and then read line by line: 7 paired writes, 34 solo writes, the rest reads, 0 references left behind. **The paired writes were the finding.** `_currentQueue` and `_displayQueue` were assigned the same value on adjacent lines in seven places and separately in nine others, so *"may these two ever disagree, and when?"* could not be answered without reading all sixteen. They disagree exactly under shuffle, where the display follows Media3's timeline — now the difference between `setQueue()` and `setPlayOrder()`, with the ten `setPlayOrder` call sites each verified to recompute the display afterwards. **Not a `@Singleton`**, unlike every other P4-E extraction: this is state that must die with the ViewModel, and a process-scoped instance would resurrect a stale queue into a fresh one. Public API unchanged — the four flows are re-exposed under their original names, so no UI file was touched. ViewModel 2 178 → 2 173 lines. **Unblocks P5-A.1, P5-B.1, P5-G.1 and P5-G.2.** Gate: **201 unit tests / 0 failures**, `assembleDebug`, `lintDebug`, `detekt`, `ktlintCheck` green. |
...
| P4-G.2 | `QueueManager` tests | 🟩 | P4-E.4 | ✅ | `QueuePolicyTest.kt` [NEW] — **22 tests, 22/22 green**, characterizing the queue persistence rules *before* the queue state is touched. `playback/QueuePolicy.kt` [NEW] extracts those rules as pure functions (no state, no Android, no coroutines) so there was something honest to test against — the rules previously lived inline in a private `suspend` method and had no reachable surface. **The ViewModel now delegates to it**, so these tests pin production behaviour rather than a parallel copy — that distinction is the whole point. Rules pinned: audio-only guard is **whole-queue, not per-item** (one video rejects the entire write, so watching a video cannot truncate the saved music session); list position → `sortOrder`; missing media dropped on restore (files get deleted between sessions); videos filtered on read as well as write; saved index clamped (stale index from a longer queue, negative index, and an index invalidated by filtering); `null` return signals the last-played-audio fallback; resume position suppressed past 99 % with `duration == 0` treated as *unknown*, not *finished*. **Negative control:** removing the video guard and the index clamp failed exactly the 5 relevant tests. Full suite **106 tests / 0 failures**. **Completed alongside P4-E.4 step 3**: `QueueManagerTest.kt` [NEW] — **11 tests, 11/11 green**, pinning the state rather than the rules. Covers the play-order/display-order relationship in both directions, that `clearQueue` deliberately leaves the index alone (the ViewModel clears them separately), null round-trips for index and context, flow/snapshot agreement, and that a handed-out snapshot is not aliased by a later write. **Negative control:** making `setPlayOrder` also write the display order — the single most likely "tidy-up" mistake — failed exactly one test, the one written to pin it. |
...
| P5-A.1 | Domain use cases | 🟩 | P4-G | ✅ | `domain/` [NEW] — `CalculateStreakUseCase`, `GetContinueWatchingUseCase`, `LogPlayEventUseCase`, plus `ContinueWatchingItem` moved out of `LibraryViewModel`. **22 tests, 22/22 green.** **Deviation from the card, deliberate:** it specified *constructor-injected DAO, single `suspend operator fun invoke`* for all three, but two of them are consumed inside reactive `combine`s. Injecting a DAO and reading it once with `first()` would have turned two live screens into stale ones — a behaviour change smuggled in by a structural refactor — so those two take the already-collected data and stay pure. `LogPlayEventUseCase` genuinely writes and *is* DAO-injected as carded. **Streak** was the prize: real arithmetic sitting inside a six-way `combine` inside a `stateIn`, reachable only by constructing the ViewModel and therefore never tested. Its rules are now pinned — a run that ended before yesterday is not a streak, today's absence is not a break, the first gap ends the count — plus the newest-first ordering it silently depends on. **Negative control:** deleting the currency check failed exactly the two tests that rest on it. Known limitation carried over unchanged: a day is a fixed 86 400 000 ms, so a DST shift can break a streak the user did not. |
| P5-A.2 | Wire use cases into ViewModels | 🟨 | P5-A.1 | ✅ *(done with A.1 — see note)* | **Merged into the A.1 commit on purpose.** Landing A.1 alone would have left the extracted logic beside the originals still running in the ViewModels — the tested copy would not have been the shipped copy, which is precisely the parallel-copy trap P4-G.2's card calls out (*"the ViewModel now delegates to it, so these tests pin production behaviour rather than a parallel copy — that distinction is the whole point"*). `AnalyticsViewModel` and `PlaybackAnalyticsTracker` now delegate, as does `LibraryViewModel` (not named on the card but it held the second copy of the continue-watching join). **`PlaybackViewModel` needed no wiring**: it only declares the `RealtimeAnalytics` data class that `AnalyticsViewModel` populates — checked rather than assumed. |
...
| P5-C.3 | `app_prefs` → DataStore (one unit) | 🟨 | P5-C.2 | ✅ **device-verified** | `data/AppPreferencesManager.kt` [NEW] — one `@Singleton` owning the **whole** `app_prefs` file, migrated by a single `SharedPreferencesMigration` with **no key list**, so every key moves in one transaction. It is the key registry the file never had: all 24 keys, their types and their defaults are declared there and nowhere else, and the four writers now name what they want (`LibrarySort.AUDIO`, `LibraryLayout.VIDEO_GRID`) rather than building key strings. Key names and encodings byte-identical to the old build. **The store is constructor-injectable, not a `preferencesDataStore` delegate** — that closes **F-37**: the upgrade path is now covered by 3 of 8 tests in one ordinary class, instead of the one-method-one-class straitjacket P5-C.2 had to accept. `AudioEffectsManager`'s separate prefs file left alone per the card. **Reads became `suspend`**, so the four consumers hydrate in `init`; the only user-visible case is the theme, where `MainActivity` now withholds the first composition until `ThemeViewModel.isLoaded` — no new dependency, no light/dark flash. Incidental: `PlaybackViewModel` lost its last use of `Application` and became a plain `ViewModel` (12 → 11 ctor params). **Gate: 186 unit tests / 0 failures** (was 178), **7/7 `connectedDebugAndroidTest` on `dragon` (API 36)**, `assembleDebug`, `lintDebug`, `detekt`, `ktlintCheck` all green; APK 28.48 MB. **The card's device check was performed, not deferred.** Built the pre-migration APK from `171ae38` in a worktree, seeded `shared_prefs/app_prefs.xml`, ran the old build (which converted the legacy `sort_video=5` ordinal in-file, giving a realistic 17-key file), then installed the new APK **over** it with `adb install -r`. Result: `shared_prefs/app_prefs.xml` deleted, `files/datastore/app_prefs.preferences_pb` created, and a protobuf-level diff of the two files shows **17/17 keys identical in name, type and value — nothing missing, nothing extra, nothing changed**. Confirmed in the UI too: the app opened light + green (`is_dark_mode=false`, `current_theme_id=green`) and the audio list showed *Sort: Title ↓* — the migrated `sort_audio_field=1` / `sort_audio_asc=false`. Write path re-verified on device: toggling a layout, playing a queue, skipping a track, and toggling shuffle/repeat/theme each landed in DataStore and survived `am force-stop` + cold launch (restored to Bravo Tone at index 1, shuffle off, repeat-one, **paused**). `setPlaylistContext(null)` genuinely **removes** the key (18 → 17) rather than storing a null. Startup was measured rather than assumed: content appears at a median 6 324 ms (new, n=9) vs 6 214 ms (old, n=3) on the same emulator — a 110 ms difference against a ~1–2 s instrument noise floor, i.e. **no measurable regression**. (`am start -W` TotalTime *favours* the new build, 3 817 vs 5 114 ms, but that is an artifact — the gate makes the first frame empty and therefore cheap — so it is not evidence of a speedup.) |
| P5-D | Gson → kotlinx.serialization | 🟨 | P4-B | ✅ | `LegacyPlaylist` is now `@Serializable`, the two `PlaylistRepository` call sites decode through one `Json {}`, and `gson` is gone from the catalog, the module and `debugRuntimeClasspath`. Plugin `org.jetbrains.kotlin.plugin.serialization` version-locked to `kotlin` (2.2.10); runtime `kotlinx-serialization-json:1.9.0`. **The parser had to be made *more* tolerant, not less.** kotlinx is strict by default where Gson was lenient, and the input is a file written by a build that no longer exists, so three settings are deliberate: `ignoreUnknownKeys` (the old React version wrote fields this model does not declare — without it *every real legacy file* fails to import and the user silently loses every playlist), `isLenient`, and `coerceInputValues` (explicit `null` → the declared default, as Gson did). `createdAt` gained a `= 0L` default for the same reason: Gson filled a missing number with 0 rather than failing. **Negative control run:** flipping `ignoreUnknownKeys` to false failed exactly one test — the one written to pin it — and nothing else, so the coverage is real rather than incidental. **Gate: 190 unit tests / 0 failures** (was 186; +4 parity tests, 11 legacy-migration tests in total), `assembleDebug`, `lintDebug`, `detekt`, `ktlintCheck` green. **APK 28.48 → 28.24 MB.** The saving is modest because `kotlinx-serialization-core` was *already* on the classpath transitively at 1.7.3 (now 1.9.0) — only the `json` artifact is genuinely new. |
| P5-E | Baseline Profile | 🟩 | P1-E | ✅ *(module + journeys; profile is a release-time artifact, deliberately not committed)* | `:baselineprofile` [NEW] — `com.android.test` + `androidx.baselineprofile`, consumed by `:app` via `baselineProfile(project(":baselineprofile"))`, plus `profileinstaller` so the profile actually applies off-Play. `BaselineProfileGenerator` covers both journeys the card names, driven by the app's own accessibility labels rather than coordinates. **Pipeline proven end-to-end**: a full `:app:generateBaselineProfile` run produced 25 902 lines, 1 879 of them FastBeat's own classes — so the journeys really executed rather than stalling on the permission screen. **That output was then deleted rather than committed**, for two reasons. (1) It was wrong: a single `collect` with `includeInStartupProfile = true` marks *everything the block touches* as startup-critical, and `startup-prof.txt` came out byte-identical to `baseline-prof.txt` with 19 211 of 25 902 entries flagged — ART prioritises the startup profile, so flagging three-quarters of the app leaves nothing to prioritise. The generator is now split into a startup-only pass and a journeys pass. (2) A committed profile is a generated artifact keyed to the generating device's media library, and it goes stale as the code moves; regenerating it belongs to a release, not to this commit. **Verification status, stated plainly:** the split's `startup` half passed on device; its `journeys` half has not completed in the same run, because the emulator's audio backend kills the emulator process when playback starts (`-no-audio` is the documented workaround, in `baselineprofile/build.gradle.kts`). The journeys themselves did run green in the pre-split run. Gate: **190 unit tests / 0 failures**, `assembleDebug`, `detekt`, `ktlintCheck` and `:baselineprofile:compileNonMinifiedReleaseKotlin` all green. |
...
| S-6.1 | Day-boundary correctness | 🟨 | — | ✅ | `domain/ObserveCurrentDayUseCase.kt` + `domain/AnalyticsDays.kt` [NEW]. The day is now a **flow** the windows are derived from, not a value read once: `AnalyticsViewModel` combined against a `MutableStateFlow` seeded at construction, so a screen left open past midnight kept crediting today's minutes to yesterday's bucket and went on highlighting yesterday as *today*. **The dead `refreshAnalytics()` is retired rather than wired up** — it was public and called from nowhere, and it could not have fixed this: resubscribing already recomputed the day, so the one case it was needed for is the one case it never covered. **Second defect found while extracting:** the chart stepped its week as `monday + n * 86_400_000` while `daily_playtime` is keyed by calendar midnight. In a zone that shifts DST mid-week every day after the shift matches no stored key and renders as a zero-height bar — it reports a day of listening as a day of silence rather than failing. `AnalyticsDays.weekOf` steps with `Calendar.add` instead. **The writer's day definition in `PlaybackAnalyticsTracker` was deliberately left duplicated, not merged:** each side owns an injectable clock, and a shared implementation would hand the tracker's tests a second hidden clock they do not control. Noted as the second occurrence per the Rule of Three. **20 new tests, 20/20 green** (`AnalyticsDaysTest` 12, `ObserveCurrentDayUseCaseTest` 8), plain JVM with the timezone pinned; the DST cases use a *synthetic* `SimpleTimeZone` shifting on a Wednesday, because real zones shift on Sundays where the bug hides, and because a JDK tzdata update must not be able to invalidate them. Virtual time drives the fake clock (`now = base + currentTime`) so the scheduler and the wall clock cannot drift apart. **Negative controls, both run:** stepping the week by a fixed day failed exactly the 3 DST tests and nothing else; reducing the ticker to a one-shot read failed exactly the 3 ticker tests. Gate: `assembleDebug lint testDebugUnitTest detekt ktlintCheck` → **BUILD SUCCESSFUL, 269 tests / 0 failures** (was 249). Neither baseline mentions the new files. |
| S-6.2 | `LibraryStats` matches its labels | 🟩 | — | ✅ | The card read **Total Storage Used** while summing audio and video only, so any library with photos in it reported a total matching nothing the user could check. **Scope deviation, deliberate:** fixing it needed one line outside the stats surface — `MediaRepository.queryImages()` never selected `MediaStore.Images.Media.SIZE`, so every image carried `size = 0` and simply adding `imageList` to the fold would have changed the number by nothing while appearing to fix it. The projection now reads `SIZE`. `LibraryStats` gains `imageCount` and a per-type storage breakdown, and **`totalStorageBytes` is now derived rather than a constructor argument**, so the headline cannot disagree with the parts — that disagreement was the whole defect. The card prints the breakdown (`Audio … · Video … · Images …`) beneath the total, which makes the figure falsifiable instead of asking to be trusted. Tiles go from 3 in one row to 4 over two rows: four 28 sp counts do not fit a phone width, and a truncated number is worse than a taller section. The fold is a top-level `internal fun libraryStatsOf`, not a lambda inside the `combine`, so the rule that was broken — *every* indexed media type is counted — is pinned by a test rather than by reading a flow declaration. **6 new tests, 6/6 green** (`LibraryStatsTest`, Robolectric only so `Uri` is real). **Negative control:** restoring the old behaviour by zeroing `imageStorageBytes` failed exactly 3 of the 6 and nothing else. ⚠️ **Verification limit, stated rather than papered over:** the `SIZE` projection itself has no unit test — there is no MediaStore on the JVM. It is covered by compilation and by inspection, and its failure mode is now visible rather than silent: if the column were ever dropped the breakdown line would read `Images 0 B` on a device that plainly has photos. Gate: `assembleDebug lint testDebugUnitTest detekt ktlintCheck` → **BUILD SUCCESSFUL, 275 tests / 0 failures** (was 269). Neither baseline mentions the changed files. |
| S-6.3 | Lifetime totals and momentum | 🟩 | S-6.1 | ✅ | `domain/PeriodChange.kt` [NEW]; `ListeningTotals` + `AnalyticsViewModel.listeningTotals`; three DAO queries; an ALL TIME card and a week-over-week chip on the *Last 7 Days* tile. Every number on the screen was a window — today, 7 days, 30 days — so a library with years of history in it had nothing to show for any of it, despite `daily_playtime` and `play_events` never being pruned. **Lifetime and momentum share one flow deliberately:** the percentage is computed from the same two totals it is drawn beside, so it cannot end up describing a week the screen is no longer showing. Five flows means the *typed* `combine`, not the vararg one — no `args[i] as Long?`, which is the only part of `realtimeAnalytics` the compiler cannot check. **`PeriodChange` is a type, not a percentage,** because the interesting case is the one a number cannot express: against a week that recorded nothing, `0%` reads as *unchanged*, a dash reads as *missing*, and a large number reads as a comparison never made. `NoBaseline` says the only true thing and the chip is simply absent. Clamping to 999% lives in the UI, not the domain — a type that caps its own output would be unusable for anything else. **Third day-boundary defect found and fixed:** `weekStart`/`monthStart` were `today - n * 86_400_000`, and walking back across a 25-hour day leaves that an hour *past* the midnight it meant to hit, so `date >= :start` drops the first day and *Last 7 Days* silently becomes six. Now `AnalyticsDays.daysBefore`. The 23-hour direction is harmless, which is why it survived — it loses data one way only, once a year. **DS-6.2 applied (Trivial):** *Active Listening* → *Audio + video*, since the tracker has always credited video and the split is refused. **23 new tests, all green** — `PeriodChangeTest` 12, `MediaDaoTest` +7 (23→30), `AnalyticsDaysTest` +4 (12→16). The DAO tests pin the empty-table NULL from `SUM` (the first launch after install, and the run least likely to be checked by hand), the `> 0` vs `> 60000` threshold difference against `getActiveDays`, and DS-6.3's deletion asymmetry. **Negative controls, three in one run, each failing exactly its own tests and nothing else:** fixed-millisecond `daysBefore` → 1; `NoBaseline` reported as `Changed(0)` → 3; first-active-day reusing the streak threshold → 2. Gate: `assembleDebug lint testDebugUnitTest detekt ktlintCheck` → **BUILD SUCCESSFUL, 298 tests / 0 failures** (was 275). Neither baseline mentions the new files. |
| S-6.4 | Activity Trends range selector | 🟨 | S-6.1 | ✅ | `domain/StatsRange.kt` + `domain/ActivityChart.kt` [NEW]; a Week/Month/Year selector above the chart. The chart was pinned to the current Mon–Sun week, so the one question it could not answer was whether this week was *normal* — last week was unreachable and so were the months of history already in `daily_playtime`. Week keeps 7 daily bars, Month is 30 daily bars, Year is 12 monthly bars. **All-time was considered and left out:** the bucket width would have to vary with how long the user has had the app, so the same chart would mean something different on every device. **The Canvas was not touched.** It already drew whatever list it was handed; making the bar type `ActivityBucket` instead of `DailyActivity` was enough, and deciding what a bucket covers moved to `ActivityChart` where it is testable without a renderer. Axis labels became data-driven and sparse — one weighted cell per bar whether or not it carries a label, since rendering only the labelled ones would space them among themselves and detach every label from the bar it names. **Range state lives in the ViewModel, not `remember`** — because it survives configuration changes and because `activityBuckets` reacts to it, so it belongs where that flow is. *(Correction: the first version of this note claimed scrolling would drop the section out of composition and reset it. That is false — the Me tab is a `Column` with `verticalScroll`, which keeps every section composed. The decision stands; the reason given for it did not.)* **Total preservation is the pinned invariant** — query bounds and buckets both come from `rangeStart`/`rangeEnd`, and the tests assert the sum survives bucketing across a February and a year boundary rather than checking individual bars. A chart that drops a day is wrong in the worst way: quietly, and looking like behaviour. `getWeekDailyPlaytimes` → `getDailyPlaytimes`; the query never was week-shaped and the name was the only thing implying the chart could not show anything else. **22 new tests** (`ActivityChartTest` 16, `AnalyticsDaysTest` +6 → 22). ⚠️ **A test bug found by its own fixture, worth recording:** a `private val` holding a fixture date ran at class-construction time, i.e. *before* `@Before` installed the fixed timezone, so it captured a midnight in the machine's own zone and matched none of the UTC day keys. Now a `get()`. Field initialisers are outside the reach of the very setup meant to make these deterministic. **Negative control:** folding each day into the previous month failed exactly the 2 total-preservation tests for the year view and nothing else. **Baseline churn, audited rather than accepted:** `detekt-baseline.xml` regenerated to +3/-10. The 3 additions are the same pre-existing `ActivityBarChart` violations re-pinned after the parameter type changed (detekt keys by signature); the 10 removals include **7 `MagicNumber` entries in `AnalyticsViewModel` that S-6.3's named window constants genuinely eliminated**. Verified by set-diffing the IDs, not by reading the diff — nothing new was silently suppressed. Gate: `assembleDebug lint testDebugUnitTest detekt ktlintCheck` → **BUILD SUCCESSFUL, 320 tests / 0 failures** (was 298). |
...
| F-7 | P0-B (code read) | `EqualizerSheet(viewModel: PlaybackViewModel, …)` takes the whole ViewModel instead of data + callbacks, contradicting audit §1.4. ~~This will make it hard to test in isolation — relevant to P5-G.~~ ✅ **Closed by P5-G.1**, and the prediction was exact: the test could not be written until `MiniPlayer` was split into a ViewModel-bound wrapper and a stateless `MiniPlayerContent`. `EqualizerSheet` still takes the whole ViewModel and will need the same treatment whenever it is tested. | ✅ Closed |
...
| F-45 | P5-A.1 | **The two continue-watching screens disagreed, and the extraction proved it.** `LibraryViewModel` fell back to the media's own duration when a history row recorded none; `AnalyticsViewModel` did not, and `MeScreen` computed `progress` inline from `history.duration` alone. The query deliberately admits `duration = 0` rows, so the same half-watched film showed real progress in the video library and a flat 0 % on the stats screen — reading as *not started*. Both now go through `GetContinueWatchingUseCase`, so this commit **changes what MeScreen renders**: a deliberate fix, not a refactor side effect, called out here because a pure-refactor task is exactly where such a change would otherwise slip by unnoticed. | ✅ Closed |
| F-44 | P5-G.1 | **A Coil `AsyncImage` silently swallows touch input under Robolectric.** The composable renders, every `assertIsDisplayed` passes, and only `performClick()` fails — with `expected:<1> but was:<0>`, which reads like the callback is mis-wired rather than like an image-loading problem. Bisected rather than guessed: a minimal clickable passed, the same clickable inside the app theme passed, the full `Surface`+shadow+clip+clickable chain passed, and adding a single `AsyncImage` took it from 1 to 0. A `FakeImageLoaderEngine` from `coil-test` did **not** restore it, so that dependency was backed out rather than left in place not earning its keep. `MiniPlayerTest` therefore invokes the `OnClick` **semantics action**, which runs the same lambda the modifier was given; what is not covered is hit-testing and overlap, which needs a device. Any future Compose test over a screen that draws artwork will hit this. | 🟡 Major |
| F-43 | P5-E | **`:baselineprofile` is not covered by detekt or ktlint.** Both plugins are applied in `:app` only, and detekt's baseline path is app-scoped, so the new module's source is unlinted. One small file today, but the gap widens with every module added. Deliberately left alone here rather than widening this task. | 🟢 Minor |
| F-42 | P5-E | **Every new Gradle module needs its own `/build` ignore line.** Root `.gitignore` has `/build` (root only) and `/app/build`; nothing matched `baselineprofile/build`, so the module's entire build output was visible to `git add -A`. Caught before committing and fixed with a root rule plus a nested `baselineprofile/.gitignore`, matching the existing `:app` pattern. This is the `/app` hazard from the `.gitignore` header inverted — that one hid sources, this one would have committed build output — and it will recur on the next module. | 🟡 Major |
| F-41 | P5-D | **`kotlinx-serialization-core` was already on `debugRuntimeClasspath` at 1.7.3 before this task**, pulled in transitively, and adding the explicit `json` dependency upgraded it to 1.9.0. Nothing depended on the older version's behaviour so the upgrade is invisible here, but it is a reminder that `libs.versions.toml` does not describe the whole classpath — the same blind spot F-15/OQ-6 flagged for the duplicate `compose.ui:ui`. | 🟢 Minor |
| F-40 | P5-C.3 (device test) | **`SharedPreferencesMigration` does not overwrite keys already in the DataStore — it only copies the ones that are absent.** Observed directly: after a downgrade-to-old-build and re-upgrade, `sort_albums_field` kept its DataStore value of `2` while `sort_albums_asc`, which the DataStore did not yet have, was copied from the legacy file the old build had just written. This is the desirable direction — stale SharedPreferences data can never clobber newer DataStore data — and it is worth knowing before writing any future migration that *expects* the legacy file to win. | 🟢 Minor |
| F-39 | P5-C.3 | **DataStore writes lose the process-death flush SharedPreferences had.** `SharedPreferences.apply()` is backed by `QueuedWork`, which the framework drains at `onPause`/`onStop`, so a queued write survived the process being killed straight afterwards. The DataStore writes replacing them are ordinary coroutines with no such guarantee. The window is small — `viewModelScope` dispatches on `Main.immediate`, so each write reaches DataStore's writer actor before the caller returns — and the highest-value case (the interrupted audio session) is written while the user is actively in the app rather than while it is being torn down. Worth a deliberate look if session loss is ever reported. | 🟢 Minor |
| F-38 | P5-C.3 | **The ktlint baseline is keyed by line number, so it breaks on any edit above a baselined line.** `app/config/ktlint/baseline.xml` pins three pre-existing `backing-property-naming` violations by line; this task shifted them twice and failed `ktlintCheck` both times, on code it never touched. Re-pinning is not the fix — deleting the violations is: `_pendingImageDeleteId` (`LibraryViewModel`, `PlaybackViewModel`) and `_pendingDeleteTrackId` (`PlaybackViewModel`) are private-only state with no public counterpart, so the underscore prefix is simply wrong, and renaming without it removes all three entries. ~~Deliberately not done here.~~ ✅ **Fixed in P4-E.4 step 3**, after it shifted and broke the build a third time. The "hot file" objection had expired — that task was already rewriting `PlaybackViewModel` — so the three fields were renamed without the underscore and all three baseline entries deleted rather than re-pinned. The last `AnalyticsViewModel` entry went the same way in P5-A.1 when adding two constructor parameters shifted it; `app/config/ktlint/baseline.xml` now carries **no** backing-property entries at all. | ✅ Closed |
| F-37 | P5-C.2 | ~~**A DataStore migration can only be tested once per JVM, and that shapes the test suite.**~~ ✅ **Closed by P5-C.3 — the constraint was the delegate, not DataStore.** `preferencesDataStore` holds a process-wide singleton whose `SharedPreferencesMigration` runs on first access; Robolectric shares one sandbox classloader across test classes on the same config, so the spent migration is shared too. Measured, not assumed: the migration test passed alone and failed in the full suite, and splitting it into its own class did **not** fix it. The workable arrangement is one class, one method, and no other test anywhere that touches this store — documented in `SortPreferencesMigrationTest`. Anyone adding a second test that reads sort preferences will silently break it. Negative control: pointing the migration at a wrong legacy filename fails the test. **P5-C.3 removes the constraint**: `AppPreferencesManager` takes its store as a constructor argument, so production keeps one instance via `@Singleton` while each test builds its own over a `TemporaryFolder` file and therefore meets an unmigrated legacy file of its own. `SortPreferencesManager` still uses the delegate and still carries the restriction — worth the same treatment next time it is touched. | ✅ Closed |
